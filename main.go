...
func (b *SwitchBrain) SetIMEMode(mode uint32) bool {
	if b.GetIMEMode() == mode { return false }
...
func (b *SwitchBrain) SetTrayHidden(hide bool) {
	...
	if err != nil { return }
	defer k.Close()
	if hide { k.SetDWordValue(RegValHideTray, 1) } else { k.SetDWordValue(RegValHideTray, 0) }
}
...
func (b *SwitchBrain) StartHotkeyListener() {
	...
	RegisterClass(className, func(hwnd uintptr, msg uint32, wparam uintptr, lparam uintptr) uintptr {
		...
		case WM_HOTKEY:
			switch int(wparam) {
			case HotkeyToggleMode: b.ToggleMode()
			case HotkeyToggleHide: b.ToggleHide()
			}
			return 0
		case WM_TRAYICON:
			if b.OnTrayEvent != nil { b.OnTrayEvent(int(lparam)) }
			return 0
		case WM_COMMAND:
			if b.OnMenuCommand != nil { b.OnMenuCommand(int(wparam & 0xFFFF)) }
			return 0
		case WM_USER + 777: b.ToggleMode(); return 0
		case WM_USER + 778: b.ToggleHide(); return 0
		case WM_CLOSE:
...
func (b *SwitchBrain) WatchRegistry(ctx context.Context, onChanged func()) {
	...
	if err != nil { k.Close(); return }
	quitEvent, err := windows.CreateEvent(nil, 0, 0, nil)
	if err != nil { windows.CloseHandle(regEvent); k.Close(); return }
	...
	for {
		...
		if err != nil || s == windows.WAIT_OBJECT_0+1 { return }
		if s == windows.WAIT_OBJECT_0 { onChanged() }
	}
}
...
func main() {
	...
	defer func() { if ret != 0 { CloseHandle(syscall.Handle(ret)) } }()
	...
	brain.OnTrayToggle = func(hide bool) {
		if hide { tray.Hide() } else { tray.Show() }
	}
